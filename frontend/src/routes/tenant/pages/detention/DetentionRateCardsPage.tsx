...
import { z } from 'zod';
import {
  createDetentionRateCard,
  deleteDetentionRateCard,
  fetchDetentionRateCards,
} from '../../../../api/endpoints/detention';
import type { DetentionRateCard } from '../../../../types';
...
export function DetentionRateCardsPage() {
  ...
  const [pendingDelete, setPendingDelete] = useState<DetentionRateCard | null>(null);

  const { data, isLoading } = useQuery({ queryKey: ['detention', 'rate-cards'], queryFn: fetchDetentionRateCards });

  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['detention', 'rate-cards'] });

  const createMutation = useMutation({
    mutationFn: createDetentionRateCard,
    onSuccess: () => {
      invalidate();
      setDialogOpen(false);
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteDetentionRateCard,
    onSuccess: () => {
      invalidate();
